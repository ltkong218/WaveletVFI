...
from models.utils import bwarp, gumbel_softmax
from models.loss import Charbonnier_L1, Ternary
...
class WaveletVFI(nn.Module):
    ...
    def sparse_decode(self, w4_l, w4_h, w3_l, c1, c2, c3, f4, thresh):
        ...
        cost3 = self.dilate(mask3).view(b, -1).mean(dim=1, keepdim=True) * (432+576) * 648 + mask3.view(b, -1).mean(dim=1, keepdim=True) * 648 * 9
        ...
        cost2 = self.dilate(mask2).view(b, -1).mean(dim=1, keepdim=True) * (288+648) * 432 + mask2.view(b, -1).mean(dim=1, keepdim=True) * 432 * 9
        ...
        cost1 = self.dilate(mask1).view(b, -1).mean(dim=1, keepdim=True) * (144+432) * 216 + mask1.view(b, -1).mean(dim=1, keepdim=True) * 216 * 9

        imgt_pred = torch.clamp(w0_l, 0, 1)
        cost = (cost1 / 4.0 + cost2 / 16.0 + cost3 / 64.0) / cost_total
        mask = upsample(mask1, 2)
        return imgt_pred, cost, mask, [w4_l, w4_h, w3_h, w2_h, w1_h]

    def forward(self, img0, img1, imgt, dynamic_train=False, thresh=None):
        ...
        wavelet_gt_list = self.waveletencoder(imgt)
        ...
        if dynamic_train:
            class_prob = gumbel_softmax(class_prob_, dim=1, tau=self.tau)
            thresh_ = self.thresh.index_select(dim=0, index=class_prob.argmax(dim=1)).view(-1, 1, 1, 1)

            imgt_pred_1, cost_1, mask_1, wavelet_pred_list_1 = self.sparse_decode(w4_l, w4_h, w3_l, c1, c2, c3, f4, self.thresh[0].view(1, 1, 1, 1))
            loss_rec_1 = l1_loss(imgt_pred_1 - imgt) + tr_loss(imgt_pred_1, imgt)
            loss_wav_1 = get_wavelet_loss(wavelet_pred_list_1, wavelet_gt_list)

            imgt_pred_2, cost_2, mask_2, wavelet_pred_list_2 = self.sparse_decode(w4_l, w4_h, w3_l, c1, c2, c3, f4, self.thresh[1].view(1, 1, 1, 1))
            loss_rec_2 = l1_loss(imgt_pred_2 - imgt) + tr_loss(imgt_pred_2, imgt)
            loss_wav_2 = get_wavelet_loss(wavelet_pred_list_2, wavelet_gt_list)

            imgt_pred_3, cost_3, mask_3, wavelet_pred_list_3 = self.sparse_decode(w4_l, w4_h, w3_l, c1, c2, c3, f4, self.thresh[2].view(1, 1, 1, 1))
            loss_rec_3 = l1_loss(imgt_pred_3 - imgt) + tr_loss(imgt_pred_3, imgt)
            loss_wav_3 = get_wavelet_loss(wavelet_pred_list_3, wavelet_gt_list)

            imgt_pred_4, cost_4, mask_4, wavelet_pred_list_4 = self.sparse_decode(w4_l, w4_h, w3_l, c1, c2, c3, f4, self.thresh[3].view(1, 1, 1, 1))
            loss_rec_4 = l1_loss(imgt_pred_4 - imgt) + tr_loss(imgt_pred_4, imgt)
            loss_wav_4 = get_wavelet_loss(wavelet_pred_list_4, wavelet_gt_list)

            loss_rec = (class_prob * torch.cat([loss_rec_1, loss_rec_2, loss_rec_3, loss_rec_4], 1)).sum(dim=1).mean()
            loss_wav = (class_prob * torch.cat([loss_wav_1, loss_wav_2, loss_wav_3, loss_wav_4], 1)).sum(dim=1).mean() * 0.01
            loss_com = (class_prob * torch.cat([loss_rec_1.detach() * cost_1, loss_rec_2.detach() * cost_2, loss_rec_3.detach() * cost_3, loss_rec_4.detach() * cost_4], 1)).sum(dim=1).mean() * 3.0

            class_prob = class_prob.unsqueeze(2).unsqueeze(3).unsqueeze(4)
            imgt_pred = (class_prob * torch.cat([imgt_pred_1.unsqueeze(1), imgt_pred_2.unsqueeze(1), imgt_pred_3.unsqueeze(1), imgt_pred_4.unsqueeze(1)], 1)).sum(dim=1)
            mask_t = (class_prob * torch.cat([mask_1.unsqueeze(1), mask_2.unsqueeze(1), mask_3.unsqueeze(1), mask_4.unsqueeze(1)], 1)).sum(dim=1)

            return imgt_pred, imgt_pred_, flow_t0, flow_t1, occ_t, mask_t, loss_rec, loss_wav, loss_com, thresh_.detach()
            ...
            imgt_pred, cost, mask_t, wavelet_pred_list = self.sparse_decode(w4_l, w4_h, w3_l, c1, c2, c3, f4, thresh_)

            loss_rec = (l1_loss(imgt_pred - imgt) + tr_loss(imgt_pred, imgt)).mean()
            loss_wav = get_wavelet_loss(wavelet_pred_list, wavelet_gt_list).mean() * 0.01
            loss_com = ((l1_loss(imgt_pred - imgt) + tr_loss(imgt_pred, imgt)).detach() * cost).mean() * 3.0

            return imgt_pred, imgt_pred_, flow_t0, flow_t1, occ_t, mask_t, loss_rec, loss_wav, loss_com, thresh_.detach()
            
    def inference(self, img0, img1, thresh=None):
        flow_t0, flow_t1, occ_t, class_prob_ = self.ifnet(img0, img1)
